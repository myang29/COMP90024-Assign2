...
# word lemmatization
def lemmatization(tweet_corpus):
    result = []
    for tweet in tweet_corpus:
        result.append([lemmatizer.lemmatize(word) for word in tweet])
    return result

def word_lower(tweet_corpus):
    result = []
    for tweet in tweet_corpus:
        result.append([word.lower() for word in tweet])
    return result
...
positive_tweets = lemmatization(positive_tweets)
negative_tweets = lemmatization(negative_tweets)

positive_tweets = word_lower(positive_tweets)
negative_tweets = word_lower(negative_tweets)
